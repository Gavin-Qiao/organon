...
Remote branch protection can also require a PR for routine changes. Follow the effective
GitHub checks and use a PR when required; this policy is not permission to bypass protection.
...
> **Review human docs before release.** Remove stale candidate-only framing from current
> usage guides, check examples and compatibility advice, and preserve historical experiment
> limits. The tag-prefix-filtered badges update themselves; versions never live in README
> prose (see `CONTRIBUTING.md`, "Docs stay truthful").
