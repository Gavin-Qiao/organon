  ...
  if (name === "sqlite-fts5") {
    ...
    return {
      ...
      update: async sources => {
        ...
        const old = new Map((db.prepare("SELECT rowid, key, title, path, body FROM units").all() as any[]).map(row => [row.key, row]));
        ...
        db.transaction(() => {
          for (const source of sources) {
            ...
            if (row) db.prepare("DELETE FROM units WHERE rowid=?").run(row.rowid);
            insert.run(source.id!, source.title, source.path, source.text); changed++;
          }
          for (const row of old.values()) { db.prepare("DELETE FROM units WHERE rowid=?").run(row.rowid); deleted++; }
        })();
        return { changed, deleted };
      },
      indexedText: async id => (db.prepare("SELECT body FROM units WHERE key=?").get(id) as { body: string } | null)?.body ?? null,
      close: async () => { db.close(); }, storage: () => treeBytes(file) + treeBytes(`${file}-wal`), workerRss: noWorker,
    };
  }
  if (name === "zvec-hybrid") {
