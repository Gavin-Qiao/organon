...
import { copyFileSync, cpSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, realpathSync, writeFileSync } from "node:fs";
...
export function stageRuntime(parent: string) {
  const runtime = realpathSync(mkdtempSync(join(parent, "publication-runtime-")));
...
export function createFixture(parent: string, runtime: ReturnType<typeof stageRuntime>, pages = 4, events = 8, ballast = 0) {
  const root = realpathSync(mkdtempSync(join(parent, "publication-fixture-")));
