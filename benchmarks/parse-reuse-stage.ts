...
`);
  ...
  transform(indexer, [["const units = collectUnits(root, vocab, cache);", 'const units = withParsedReuse(root, vocab, argv.includes("--source-hash"), () => collectUnits(root, vocab, cache));']]);
