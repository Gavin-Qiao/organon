...
| lay down / rebuild the workspace for a venue | `bun "<plugin-root>/scripts/editio-scaffold.ts" --venue arxiv|tpami|nmi|neurips` |
...
| check the workspace against the installed plugin (scaffold/order/venue/asset drift, stale/unwired sections, identity/path leaks, VCS hazards, and venue-specific page/word/display/file budgets) | `bun "<plugin-root>/scripts/editio-doctor.ts"` (report-only; `--strict` exits 1 for CI) |
...
additionally masks `\selfcite` and drops `\blindhide{…}` blocks. Ordinary class profiles
swap the author block for the anonymous label from `paper.json`; an official venue package
may own anonymity instead. Venue remains data: class/package, mode mapping, assembly, assets,
and budgets all live in `venue.json`.
