...
export function assertSnapshot(root: string) {
  ...
  if (!path.startsWith(temp + "/") || lstatSync(root).isSymbolicLink()) throw new Error("snapshot must be a physical OS-temp directory");
...
export function capture(sourceInput: string, label: string) {
  ...
  const root = mkdtempSync(join(tmpdir(), `organon-private-${label}-`));
