...
const parent = realpathSync(mkdtempSync(join(tmpdir(), "organon-private-parse-test-")));
...
test("subprocess timeout kills its process group and keeps raw diagnostics private", async () => {
  ...
  if (process.platform !== "linux") {
    await expect(boundedCommand([process.execPath, "-e", "process.exit(0)"], log)).rejects.toThrow("private-trial-platform");
    expect(existsSync(log)).toBe(false); return;
  }
