# Local retrieval: architecture and evidence

Decision: retain Promptus lexical retrieval as the default, with an optional,
local QMD vector route for conceptual recall. The adapter is implemented; activation
requires explicit per-project configuration and separately staged dependencies.
No installation acquires an embedding model or vector database automatically.
